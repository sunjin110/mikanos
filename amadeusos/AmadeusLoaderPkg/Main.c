...
// SaveMemoryMap メモリマップをファイルに保存する
EFI_STATUS SaveMemoryMap(struct MemoryMap* map, EFI_FILE_PROTOCOL* file) {
    ...
    EFI_PHYSICAL_ADDRESS iter; // 指し記したいメモリマップのアドレスを整数でiterする
    int i;
    for (iter = (EFI_PHYSICAL_ADDRESS)map->buffer, i = 0; iter < (EFI_PHYSICAL_ADDRESS)map->buffer + map->map_size; iter += map->descriptor_size, i++) {
        ...
        EFI_MEMORY_DESCRIPTOR* desc = (EFI_MEMORY_DESCRIPTOR*)iter; // iterを実際のメモリの場所を指すpointerに変換する
